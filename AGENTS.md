...
- `from_schema()` sets `self._context = None` in its `finally` block. Direct method calls (e.g. `jsonschema_string()`) must tolerate this.
...
- Test both direct method calls (`faker.jsonschema_object(...)`) and `faker.from_schema({...})` round-trips.
